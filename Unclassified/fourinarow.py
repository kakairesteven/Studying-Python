...
# Contants used for displaying the board
EMPTY_SPACE = '.'# A period that is easier to count than a space.
PLAYER_X = 'X'
PLAYER_0 = '0'

# Note: Update displayBoard() and COLUMN_LABEL if BOARD_WIDTH is changed.
BOARD_WIDTH = 7
BOARD_HEIGHT = 6
...
def main():
    print("""Four in a row,
    Two players take turns dropping tiles into one of the seven
    columns, trying to make four in a row horizontally, vertically, or diagonally.""")
    ...
    while True: # Run a player's turn.
        ...
        if isWinner(playerTurn, gameBoard):
            displayBoard(gameBoard) # Display the board one last timex
...
def getNewBoard():
    """Returns a dictionary that represents a Four in a
    row board.
    The keys are (columnIndex, rowIndex) tuples of two integers, and 
    the value are one of 'X', '0', or '.' (empty space) stringed
    """
    board = {}
...
def displayBoard(board):
    ...
    '''Prepare a list to pass to the format() string method for
    the board template. The list holds all of the board's
    tiles (and empty spaces) going left to right, top to bottom.
    '''
...
def askForPlayerMove(playerTile, board):
    """
    Let a player select a column on the board to drop
    a tile into:
...
def isFull(board):
    ...
    for rowIndex in range(BOARD_HEIGHT):
        for columnIndex in range(BOARD_WIDTH):
            if board[(columnIndex, rowIndex)] == EMPTY_SPACE:
                return False # Found empty space, so return False

...
def main():
    ...
    adjacent sqaures of that color/shape. Try to make the
    entire board the same color/shape.
    ''')

    print('Do you prefer playing in colorblind? Y/N')
    response = input('> ')
...
def getNewBoard():
    ...
    # Make several tiles the same as their neighbor. This creates
    # groups of the same color/shape
...
def displayBoard(board, displayMode):
    """
    Display the board on the the screen."""
    bext.fg('white')
    ...
    for y in range(BOARD_HEIGHT):
        ...
        if y == 0: # The first row begins with '>'.
            print('>', end='')
        else: # Later rows begins with a white vertical line.
            print(UPDOWN, end='')
...
def askForPlayerMove(displayMode):
    """
    Let the player select a color to plant the upper left tile.
    """
    while True:
        bext.fg('white')
...
def changeTile(tileType, board, x, y, charToChange=None):
    """Change the color/shape of a tile using the recursive
